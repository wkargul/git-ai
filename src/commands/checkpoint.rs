...
/// Maximum age of a file's mtime (in seconds) for the race-condition heuristic.
/// If a Human checkpoint's target file was modified within this window, we assume
/// the non-blocking agent (e.g. Codex `notify`) already wrote the file before
/// the checkpoint ran, and promote the checkpoint to AiAgent.
const MTIME_RACE_WINDOW_SECS: u64 = 5;
...
/// Check whether any file in `paths` was modified within the race-condition
/// window (`MTIME_RACE_WINDOW_SECS`).  Paths may be relative (resolved against
/// `repo_workdir`) or absolute.  Returns `true` as soon as *any* path has a
/// recent mtime; returns `false` if none do or if metadata cannot be read.
///
/// This is intentionally best-effort: on platforms where `mtime` resolution is
/// coarse (e.g. FAT32 with 2-second granularity) the 5-second window is wide
/// enough to still catch races.
fn any_file_modified_within_race_window(repo: &Repository, paths: &[String]) -> bool {
    let repo_workdir = match repo.workdir() {
        Ok(dir) => dir,
        Err(_) => return false,
    };
    let now = SystemTime::now();
    let threshold = StdDuration::from_secs(MTIME_RACE_WINDOW_SECS);
    ...
    for path in paths {
        ...
        {
            // On some platforms mtime could theoretically be ahead of
            // `now` (clock skew, NFS, etc.).  Treat that as "recent".
            let elapsed = now.duration_since(modified).unwrap_or(StdDuration::ZERO);
            if elapsed < threshold {
                debug_log(&format!(
                    "mtime race detected: {} modified {:.2}s ago (threshold {}s)",
                    path,
                    elapsed.as_secs_f64(),
                    MTIME_RACE_WINDOW_SECS,
                ));
                return true;
            }
        }
...
/// If the checkpoint is a Human checkpoint whose `will_edit_filepaths` contain
/// at least one recently-modified file, promote it to an AiAgent checkpoint.
/// This handles the race where a non-blocking agent (e.g. Codex `notify`)
/// already wrote the file before the Human checkpoint ran.
///
/// Returns the (possibly promoted) `kind` and `agent_run_result`.
fn maybe_promote_human_to_ai(
    repo: &Repository,
    kind: CheckpointKind,
    agent_run_result: Option<AgentRunResult>,
) -> (CheckpointKind, Option<AgentRunResult>) {
    if kind != CheckpointKind::Human {
        return (kind, agent_run_result);
    }
    let Some(ref result) = agent_run_result else {
        return (kind, agent_run_result);
    };
    let Some(ref paths) = result.will_edit_filepaths else {
        return (kind, agent_run_result);
    };
    if paths.is_empty() {
        return (kind, agent_run_result);
    ...
    if any_file_modified_within_race_window(repo, paths) {
        debug_log(
            "Promoting Human checkpoint to AiAgent: target file(s) have recent mtime \
             (likely already written by agent)",
        );
        let mut promoted = agent_run_result.unwrap(); // safe: checked Some above
        promoted.checkpoint_kind = CheckpointKind::AiAgent;
        promoted.edited_filepaths = promoted.will_edit_filepaths.take();
        (CheckpointKind::AiAgent, Some(promoted))
    } else {
...
pub(crate) fn run_with_base_commit_override_with_policy(
    ...
    // Detect mtime race: if a Human checkpoint's target files were already
    // written by the agent, promote to AiAgent so attribution is correct.
    let (kind, agent_run_result) = maybe_promote_human_to_ai(repo, kind, agent_run_result);
...
mod tests {
    ...
    // ---- mtime race-condition promotion tests ----

    #[test]
    fn test_maybe_promote_human_to_ai_skips_non_human_checkpoint() {
        let tmp = TmpRepo::new().expect("TmpRepo should create");
        let result = test_agent_run_result(
            CheckpointKind::AiAgent,
            Some(vec!["src/main.rs"]),
            None,
            None,
        ...
        let (kind, returned) =
            maybe_promote_human_to_ai(tmp.gitai_repo(), CheckpointKind::AiAgent, Some(result));
        assert_eq!(
            kind,
            CheckpointKind::AiAgent,
            "AiAgent should remain AiAgent"
        );
        assert!(returned.is_some());
        assert!(
            returned.as_ref().unwrap().edited_filepaths.is_some(),
            "edited_filepaths should be unchanged"
        );
    }

    #[test]
    fn test_maybe_promote_human_to_ai_skips_none_agent_result() {
        let tmp = TmpRepo::new().expect("TmpRepo should create");
        let (kind, returned) =
            maybe_promote_human_to_ai(tmp.gitai_repo(), CheckpointKind::Human, None);
        assert_eq!(
            kind,
            CheckpointKind::Human,
            "Human with no agent result stays Human"
        );
        assert!(returned.is_none());
    }

    #[test]
    fn test_maybe_promote_human_to_ai_skips_empty_will_edit() {
        let tmp = TmpRepo::new().expect("TmpRepo should create");
        let result = test_agent_run_result(CheckpointKind::Human, None, Some(vec![]), None);
        let (kind, _) =
            maybe_promote_human_to_ai(tmp.gitai_repo(), CheckpointKind::Human, Some(result));
        assert_eq!(
            kind,
            CheckpointKind::Human,
            "Empty will_edit_filepaths stays Human"
        );
    }

    #[test]
    fn test_maybe_promote_human_to_ai_skips_no_will_edit() {
        let tmp = TmpRepo::new().expect("TmpRepo should create");
        let result = test_agent_run_result(CheckpointKind::Human, None, None, None);
        let (kind, _) =
            maybe_promote_human_to_ai(tmp.gitai_repo(), CheckpointKind::Human, Some(result));
        assert_eq!(
            kind,
            CheckpointKind::Human,
            "None will_edit_filepaths stays Human"
        );
    }

    #[test]
    fn test_maybe_promote_human_to_ai_promotes_recent_file() {
        let tmp = TmpRepo::new().expect("TmpRepo should create");
        // Write a file *just now* so its mtime is within the 5-second window.
        tmp.write_file("recent.txt", "hello\n", false)
            .expect("write should succeed");

        let result =
            test_agent_run_result(CheckpointKind::Human, None, Some(vec!["recent.txt"]), None);
        let (kind, returned) =
            maybe_promote_human_to_ai(tmp.gitai_repo(), CheckpointKind::Human, Some(result));
        assert_eq!(
            kind,
            CheckpointKind::AiAgent,
            "Recent mtime should promote Human to AiAgent"
        );
        let agent = returned.expect("promoted result should be Some");
        assert_eq!(agent.checkpoint_kind, CheckpointKind::AiAgent);
        assert!(
            agent.edited_filepaths.is_some(),
            "will_edit_filepaths should be moved to edited_filepaths"
        );
        assert!(
            agent.will_edit_filepaths.is_none(),
            "will_edit_filepaths should be None after promotion"
        );
        assert_eq!(
            agent.edited_filepaths.as_ref().unwrap(),
            &vec!["recent.txt".to_string()],
        );
    }

    #[test]
    fn test_maybe_promote_human_to_ai_keeps_old_file() {
        let tmp = TmpRepo::new().expect("TmpRepo should create");
        ...
        filetime::set_file_mtime(&file_path, old_time).expect("set_file_mtime should succeed");

        let result =
            test_agent_run_result(CheckpointKind::Human, None, Some(vec!["old.txt"]), None);
        let (kind, _) =
            maybe_promote_human_to_ai(tmp.gitai_repo(), CheckpointKind::Human, Some(result));
        assert_eq!(
            kind,
            CheckpointKind::Human,
            "Old mtime should keep Human checkpoint"
        );
    }

    #[test]
    fn test_maybe_promote_human_to_ai_nonexistent_file_stays_human() {
        let tmp = TmpRepo::new().expect("TmpRepo should create");
        let result = test_agent_run_result(
            CheckpointKind::Human,
            None,
            Some(vec!["does-not-exist.txt"]),
            None,
        );
        let (kind, _) =
            maybe_promote_human_to_ai(tmp.gitai_repo(), CheckpointKind::Human, Some(result));
        assert_eq!(
            kind,
            CheckpointKind::Human,
            "Nonexistent file should not trigger promotion"
        );
    }

    #[test]
    fn test_any_file_modified_within_race_window_absolute_path() {
        let tmp = TmpRepo::new().expect("TmpRepo should create");
        let file_path = tmp.path().join("abs.txt");
        std::fs::write(&file_path, "content\n").expect("write should succeed");

        let abs_string = file_path.to_string_lossy().to_string();
        assert!(
            any_file_modified_within_race_window(tmp.gitai_repo(), &[abs_string]),
            "Freshly-written file via absolute path should be detected as recent"
        );
    }

    #[test]
    fn test_any_file_modified_within_race_window_mixed_old_and_new() {
        let tmp = TmpRepo::new().expect("TmpRepo should create");
        ...
        filetime::set_file_mtime(&old_path, old_time).expect("set_file_mtime should succeed");

        // New file
        tmp.write_file("new.txt", "new\n", false)
            .expect("write should succeed");

        let paths = vec!["old.txt".to_string(), "new.txt".to_string()];
        assert!(
            any_file_modified_within_race_window(tmp.gitai_repo(), &paths),
            "Should return true when at least one file has recent mtime"
        );
    }

    #[test]
    fn test_any_file_modified_within_race_window_all_old_returns_false() {
        let tmp = TmpRepo::new().expect("TmpRepo should create");
        ...
        filetime::set_file_mtime(&p1, old).expect("set mtime");
        filetime::set_file_mtime(&p2, old).expect("set mtime");

        let paths = vec!["a.txt".to_string(), "b.txt".to_string()];
        assert!(
            !any_file_modified_within_race_window(tmp.gitai_repo(), &paths),
            "All-old files should return false"
        );
    }
}
